...
<html>
	...
	<body data-type="generalComponents">
							...
							<div class="am-u-sm-12 am-u-md-9">
								<form class="am-form am-form-horizontal">
									<div class="am-form-group">
										...
										创建人用户 / Creation</label>
										<div class="am-u-sm-9">
											<input type="text" id="user-name" placeholder="创建人用户 / Creation" class="Creation" value='${user.creation }'> <small>输入创建人用户 /
											Creation</small>
										</div>
									</div>
									...
									<div class="am-form-group">
										...
										/ CreationName</label>
										<div class="am-u-sm-9">
											<input type="text" id="user-name" placeholder="创建人姓名 / CreationName" class="CreationName" value='${user.creationname }'> <small>输入创建人姓名
											/ CreationName</small>
										</div>
									</div>
									<div class="am-form-group">
										<label for="user-name" class="am-u-sm-3 am-form-label">客户名称
										/ ClientName</label>
										<div class="am-u-sm-9">
											<input type="text" id="user-name" placeholder="客户公司 / ClientCompany" class="ClientName" value='${user.contact }'> <small>客户名称 /
											ClientCompany</small>
										</div>
									</div>

									<div class="am-form-group">
										<label for="user-name" class="am-u-sm-3 am-form-label">客户来源
										/ Source</label>
										<div class="am-u-sm-9">
											<select data-am-selected="{maxHeight: 100}" class="ClientSource">
												<c:if test="${user.source == 0 }">
													<optgroup label="客户来源">
														<option value="0" selected>自主客户</option>
														<option value="1">媒体宣传</option>
														<option value="2">招标伙伴</option>
													</optgroup>
												</c:if>
												<c:if test="${user.source == 1 }">
													<optgroup label="客户来源">
														<option value="0">自主客户</option>
														<option value="1" selected>媒体宣传</option>
														<option value="2">招标伙伴</option>
													</optgroup>
												</c:if>
												<c:if test="${user.source == 2 }">
													<optgroup label="客户来源">
														<option value="0">自主客户</option>
														<option value="1">媒体宣传</option>
														<option value="2" selected>招标伙伴</option>
													</optgroup>
												</c:if>
									...
									<div class="am-form-group">
										<label for="user-name" class="am-u-sm-3 am-form-label">客户公司
										/ ClientCompany</label>
										<div class="am-u-sm-9">
											<input type="text" id="user-name" placeholder="客户公司 / ClientCompany" class="ClientCompany" value='${user.company}'> <small>客户公司 /
											ClientCompany</small>
										</div>
									</div>
									<div class="am-form-group">
										<label for="user-name" class="am-u-sm-3 am-form-label">客户性别
										/ ClientSex</label>
										<div class="am-u-sm-9">
											<select data-am-selected class="sex">
												<c:if test="${user.contactsex == 0 }">
													<optgroup label="性别">
														<option value="0" selected>男</option>
														<option value="1">女</option>
													</optgroup>
												</c:if>
												<c:if test="${user.contactsex == 1 }">
													<optgroup label="性别">
									...
									<div class="am-form-group">
										<label for="user-name" class="am-u-sm-3 am-form-label">客户等级
										/ ClientGrade</label>
										<div class="am-u-sm-9">
											<select data-am-selected class="ClientGrade">
												<c:if test="${user.clientgrade == 0 }">
													<optgroup label="客户等级">
														<option value="0" selected>超级会员</option>
														<option value="1">钻石会员</option>
														<option value="2">会员</option>
													</optgroup>
												</c:if>
												<c:if test="${user.clientgrade == 1 }">
													<optgroup label="客户等级">
														<option value="0">超级会员</option>
														<option value="1" selected>钻石会员</option>
														<option value="2">会员</option>
													</optgroup>
												</c:if>
												<c:if test="${user.clientgrade == 2 }">
													<optgroup label="客户等级">
														<option value="0" selected>超级会员</option>
														<option value="1">钻石会员</option>
														<option value="2" selected>会员</option>
													</optgroup>
												</c:if>
											</select>
										</div>
									...
									<div class="am-form-group">
										...
										/ Age</label>
										<div class="am-u-sm-9">
											<input type="text" id="user-name" placeholder="年龄 / Age" value='${user.contactage }' class="Age"> <small>年龄
											/ Age</small>
										</div>
									</div>
									...
									<div class="am-form-group">
										...
										/ Phone</label>
										<div class="am-u-sm-9">
											<input type="text" id="user-name" placeholder="电话号码 / Phone" class="Phone" value='${user.contactphone }'> <small>电话号码
											/ Phone</small>
										</div>
									</div>

									<div class="am-form-group">
										<label for="user-name" class="am-u-sm-3 am-form-label">其他联系方式
										/ OtherContacts</label>
										<div class="am-u-sm-9">
											<input type="text" id="user-name" value='${user.contactelse }' placeholder="其他联系方式 / OtherContacts" class="OtherContacts">
											<small>其他联系方式 / OtherContacts</small>
										</div>
									</div>
		...
		<script type="text/javascript">
			// 资源管理>新增>提交
			$('#sumbitInsert').on('click', function() {
				var urlname = window.location.search;
				var id = urlname.replace(/[^0-9]/g, "");
				var Creation = $('.Creation').val();
				var CreationName = $('.CreationName').val();
				var ClientName = $('.ClientName').val();
				var ClientSource = $('.ClientSource').val();
				var ClientCompany = $('.ClientCompany').val();
				var sex = $('.sex').val();
				var ClientGrade = $('.ClientGrade').val();
				var Age = $('.Age').val();
				var Phone = $('.Phone').val();
				var OtherContacts = $('.OtherContacts').val();
				var myreg = /^(((13[0-9]{1})|(14[0-9]{1})|(17[0]{1})|(15[0-3]{1})|(15[5-9]{1})|(18[0-9]{1}))+\d{8})$/;
				var regAge = /^\d{1,2}$/;
				if((Creation == null || Creation == "") || (CreationName == null || CreationName == "") ||
					(ClientName == null || ClientName == "") || (ClientSource == null || ClientSource == "") ||
					(ClientCompany == null || ClientCompany == "") ||
					(sex == null || sex == "") ||
					(ClientGrade == null || ClientGrade == "") ||
					(Age == null || Age == "") ||
					(Phone == null || Phone == "")) {

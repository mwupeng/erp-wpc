...
<html>
	...
	<body data-type="generalComponents">
		...
		<div class="tpl-page-container tpl-page-header-fixed">
			...
			<div class="tpl-left-nav tpl-left-nav-hover" style="margin-left: 15px; margin-right: 15px; min-height: 750px;">
				<div class="tpl-left-nav-title">用 户 列 表</div>
				<div class="tpl-left-nav-list">
					<ul class="tpl-left-nav-menu">
						<c:forEach var="i" items="${Clientlist }">
							<li class="tpl-left-nav-item" onclick="ClientName('${i.contact }')">
									...
									<table class="am-table am-table-striped am-table-hover table-main">
										...
										<tbody>
											<c:if test="${address !=  null }">
												<c:forEach items="${address }" var="list">
													<tr>
														...
														<td>${list.consigneephone }</td>
														<td class="am-hide-sm-only">${list.creationtime }</td>
														<td>
															<div class="am-btn-toolbar">
																<div class="am-btn-group am-btn-group-xs">
																	<a href='/clientaddress/ExitIndex?id=${list.id }'><input type="button" class="am-btn am-btn-default am-btn-xs " value="编辑" /></a>
											...
											<c:if test="${Client !=  null }">
												<c:forEach items="${Client }" var="list">
													<tr>
														<td><input type="checkbox" name="checkboxId" class="checkboxId" value='${list.clientaddress.id }'></td>
														<td id="listId">${list.clientaddress.id }</td>
														<td>${list.clientaddress.creation }</td>
														<td>${list.clientaddress.creationame }</td>
														<td>${list.clientaddress.status }</td>
														<c:if test="${list.clientaddress.state ==  0 }">
															<td>正常</td>
														</c:if>
														<c:if test="${list.clientaddress.state ==  1 }">
															<td>失效</td>
														</c:if>
														<td>${list.clientaddress.client.contact }</td>
														<td>${list.clientaddress.address }</td>
														<td>${list.clientaddress.consignee }</td>
														<td>${list.clientaddress.consigneephone }</td>
														<td class="am-hide-sm-only">${list.clientaddress.creationtime }</td>
														<td>
															<div class="am-btn-toolbar">
																<div class="am-btn-group am-btn-group-xs">
																	<a href='/client/Exit?id=${list.id }'><input type="button" class="am-btn am-btn-default am-btn-xs " value="编辑" /></a>
																	<a><input type="button" class="am-btn am-btn-default am-btn-xs" value="删除" onclick="del(${list.id })" /></a>
		...
		<script type="text/javascript">
			...
			$('#ClientInsert').on('click', function() {
				...
				if(urlname == null || urlname == "") {
					...
					$(window).attr(
						'location',
						'/clientaddress/AddressInindex' + urlname);
				}

	...
	<body data-type="generalComponents">
							...
							<div class="am-u-sm-12 am-u-md-6">
								<div class="am-btn-toolbar">
									<div class="am-btn-group am-btn-group-xs">
										<a><button type="button" class="am-btn am-btn-default am-btn-success" onclick="commodityrefer()">
											<span class="am-icon-plus"></span> 确定提交
										</button></a>
									</div>
									...
									<table class="am-table am-table-striped am-table-hover table-main">
										<thead>
											<tr>
												...
												<th class="table-type">商品单价</th>
												<th class="table-type">订购数量</th>
										...
										<tbody>
											<c:forEach items="${list }" var="list">
												<tr>
													<td class="listId">${list.commodity.id }</td>
													<td>
														<a>${list.commodity.classify}</a>
													</td>
													<td>${list.commodity.commodityname}</td>
													<td>${list.commodity.inventory}</td>
													<td>${list.commodity.commodityprice}</td>
													<td>
														<div class="am-u-lg-6">
															<div class="am-input-group am-input-group-sm">
																<input type="text" class="am-form-field Commoditymath" value="${list.commoditymath}">
															</div>
														</div>
													</td>
		...
		<div class="am-modal am-modal-alert" tabindex="-1" id="my-info">
			<div class="am-modal-dialog">
				...
				<div class="am-modal-bd">数量已经大于库存需要通知仓库</div>
		...
		<div class="am-modal am-modal-alert" tabindex="-1" id="my-error">
			<div class="am-modal-dialog">
				<div class="am-modal-hd">注意!</div>
				<div class="am-modal-bd">商品未有数量</div>
				<div class="am-modal-footer">
					<span class="am-modal-btn">确定</span>
				</div>
			</div>
		</div>
		<div class="am-modal am-modal-alert" tabindex="-1" id="my-commodity">
			<div class="am-modal-dialog">
				<div class="am-modal-hd">注意!</div>
		...
		<script type="text/javascript">
			...
			function commodityrefer() {

				var price = 0.0;
				$('.Commoditymath:input').each(function(index, element) {
					if($(this).val() == 0 || $(this).val() == "" || $(this).val() == null) {
						...
						var id = $('.listId').get(index).innerHTML;
						var count = $(this).val();
						if((commodityList != null || commodityList != "") && (Id != null || Id != "")) {
							Id.push(id)
							commodityList.push(count);
							var money = $('.PriceCount').get(index).innerHTML;
							price = price + (money * count);
							$.ajax({
								url: "/salesorder/CommdityList",
								data: {
									CommotidyId: Id,
									Count: commodityList
								},
								success: function(data) {
									if(data) {
										$('#my-commodity').modal('open');
										commodityList = [];
										Id = [];
										$('.am-modal-btn').on(
											'click',
											function() {
												window.close("/salesorder/CommdityIndex");
											})
									}
								},
								dataType: "text",
